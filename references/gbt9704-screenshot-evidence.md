...
| 7.2.4 | 机关全称/规范简称可加“文件”，居中红色；上边缘距版心上边缘35mm；联合行文主办机关在前，联署名称上下居中。 | `formal-digital-all-1.png`、`formal-joint-1.png`；坐标日志显示机关标志 71.41mm 页面坐标，字段差0.00mm；联合行文以主办机关在前、联署机关名称分行排列，“文件”置右并按联署名称上下居中；名称过长时自适应标志字号。 | 截图通过，推荐小标宋字体需目标机复核；复杂机关专用标志仍需模板核对。 |
...
2. 联合行文曾把联署机关名称错误压成同一横行；现在主办机关在前、联署机关名称分行排列，“文件”置右并按联署名称上下居中，名称过长时自适应标志字号。
